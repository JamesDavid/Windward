...
function loadPatched(favorCost) {  // favorCost here = the divisor under sweep
  ...
  for (const f of SRC) {
    ...
    if (f === '01_config.js') {
      code = code.replace(/FAVOR_CELL_DIVISOR: \d+/, 'FAVOR_CELL_DIVISOR: ' + favorCost);
    }
...
const PRICES = [4, 8, 16, 32, 64];   // FAVOR_CELL_DIVISOR candidates (higher = leaner income)
...
console.log('BEST DIVISOR:', best.price, '(avg', best.avg + ')');
