...
function playerAct(t) {
  ...
  if (p.state === 'idle' && !supply.temple && p.islandId !== supply.id) {
    ...
    const plotIdx = supply.plots.findIndex(pl => !pl.structure && !segTouches(pl) && !G.plotBlockedByQuarry(supply, pl));
  ...
  for (const isl of state.map.islands) {
    ...
    if (!guarded && state.res.A.supply >= 7 + 4) {
      const pi = isl.plots.findIndex(pl => !pl.structure && !segTouches(pl) && !G.plotBlockedByQuarry(isl, pl));
