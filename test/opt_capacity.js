...
function loadPatched(favorCost) {
  ...
  for (const f of SRC) {
    ...
    if (f === '01_config.js') {
      code = code.replace(/HOTAIR_CAPACITY: \d+/, 'HOTAIR_CAPACITY: ' + favorCost);
      code = code.replace(/HYDROGEN_CAPACITY: \d+/, 'HYDROGEN_CAPACITY: ' + (favorCost * 2));
    }
...
function runTrial(favorCost, seed) {
  const G = loadPatched(favorCost);
  ...
  let banked = 0, lostCargo = 0;
  G.Events.on('delivery', (p) => { if (p.side === 'A') banked += p.amount || 0; });
  G.Events.on('convoyLost', (p) => { if (p.ent && p.ent.owner === 'A') lostCargo += p.ent.cargo || 0; });
...
function score(r) {
  ...
  s += Math.min((r.banked || 0) / 40, 15);           // throughput matters
  if ((r.supplyEnd || 0) > 600) s -= 10;             // supply flood: capacity outruns spending
  if ((r.lostCargo || 0) > 250) s -= 8;              // convoys too heavy: losses feel unfair
  s += r.temples >= 1 ? 5 : -5;
  s += r.segsP >= 50 ? 5 : -5;
...
const PRICES = [6, 12, 24, 48, 96];   // hot-air capacity; hydrogen = 2x
